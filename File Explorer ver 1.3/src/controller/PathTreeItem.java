...
public class PathTreeItem extends TreeItem<Path>{
	...
	private void wczytaj_katalog() throws IOException {	//if we are dealing with a directory
	...
	public void delete() {
		if(this.isLeaf()) {
			...
			//-------------- Erasing the disk - for now do without it
			try {
	...
	public void create(String name, boolean plik) {
		...
		String pathFile;
		Path newPath;
		if(isDirectory) { // if you point to the directory
			...
			System.out.println("Pointing to the file ! The folder in which the file is to be created .");
		}
	}
}